  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button variant="text" className="cancelBtn" onClick={handleCancelBtn}>
          Cancel
        </Button>
      </Stack>
    </Box>
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    // if (!token) return;
    try {
      // TODO: CRIO_TASK_MODULE_CHECKOUT -  new address to the backend and display the latest list of addresses
      ...
      // console.log(`post` + response.data[35].address.value);
      // console.log(`post` + response.data);

      // setNewAddress({ isAddingNewAddress: true, value: response.data });
      setAddresses({ ...addresses, all: response.data });
  ...
  // console.log(addresses.all); //address array ;
  /**
  ...
  const deleteAddress = async (token, addressId) => {
    ...
    try {
      ...
      // let latestAddress = response.data;
      // getAddresses(token);
      setAddresses({ ...addresses, all: response.data });
    } catch (e) {
      if (e.response) {
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      const netBalance = balance - getTotalCartValue(items);
      localStorage.balance = netBalance;
      history.push("/thanks");
      return true;
    } catch (e) {
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Dislay either "Add new address" button or the <So> component to edit the currently selected address */}
